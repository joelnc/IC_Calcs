...
wBasins = sdwPath + "sdw.DBO.WatershedBasins_py" 
...
with arcpy.da.SearchCursor(wBasins, ["OBJECTID", "NAME", "Shape.STArea()"]) as cursor:
    for row in cursor:
        ...
        sel_Wshed = cf.buildWhereClause(tempWsheds, "NAME", currWshed)
        arcpy.SelectLayerByAttribute_management(
            tempWsheds, "NEW_SELECTION", sel_Wshed)

        # new shp
        outShape = "c:/Users/95218/Python/TMDL_IC_Calcs/testFolder/" + currWshed + ".shp"
...
# Write csv
outputDataFrame.to_csv(localDir + "IC_summary_table.csv", index=False)
